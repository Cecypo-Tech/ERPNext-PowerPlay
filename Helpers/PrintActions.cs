...
namespace ERPNext_PowerPlay.Helpers
{
    public class PrintActions
    {
                ...
                if (PrinterExists(printrow.Printer))
                {
                    ...
                    switch (printrow.PrintEngine)
                    {
                        ...
                        case PrintEngine.CustomTemplate:
                            ...
                            // Enrich JSON: strip HTML from item descriptions and add linked payments
                            jsonDoc = await EnrichJsonDocument(jsonDoc, doctype, doc.Name);
                ...
                using (JsonDocument document = JsonDocument.Parse(jsonDoc))
                {
                    ...
                    using (var stream = new MemoryStream())
                    {
                        using (var writer = new Utf8JsonWriter(stream, options))
                        {
                            ...
                            var linkedPayments = await GetLinkedPayments(doctype, docname);
        ...
        private async Task<List<LinkedPayment>> GetLinkedPayments(string doctype, string docname)
        {
            var payments = new List<LinkedPayment>();
            try
            {
                var frappeApi = new FrappeAPI();
                string refJson = await frappeApi.GetLinkedPaymentEntries(doctype, docname);

                if (string.IsNullOrEmpty(refJson)) return payments;

                using (JsonDocument refDoc = JsonDocument.Parse(refJson))
                {
                    var dataArray = JsonHelper.GetJsonElement(refDoc.RootElement, "data");
                    if (dataArray.ValueKind != JsonValueKind.Array) return payments;

                    foreach (var refEntry in dataArray.EnumerateArray())
                    {
                        var parentElement = JsonHelper.GetJsonElement(refEntry, "parent");
                        var allocatedElement = JsonHelper.GetJsonElement(refEntry, "allocated_amount");

                        string parentName = JsonHelper.GetJsonElementValue(parentElement);
                        double allocatedAmount = 0;
                        if (allocatedElement.ValueKind == JsonValueKind.Number)
                            allocatedAmount = allocatedElement.GetDouble();

                        if (string.IsNullOrEmpty(parentName)) continue;

                        // Fetch Payment Entry details
                        string peJson = await frappeApi.GetPaymentEntryDetails(parentName);
                        if (string.IsNullOrEmpty(peJson)) continue;

                        using (JsonDocument peDoc = JsonDocument.Parse(peJson))
                        {
                            var peData = JsonHelper.GetJsonElement(peDoc.RootElement, "data");

                            var postingDate = JsonHelper.GetJsonElementValue(JsonHelper.GetJsonElement(peData, "posting_date"));
                            var paidAmountEl = JsonHelper.GetJsonElement(peData, "paid_amount");
                            var modeOfPayment = JsonHelper.GetJsonElementValue(JsonHelper.GetJsonElement(peData, "mode_of_payment"));
                            var referenceNo = JsonHelper.GetJsonElementValue(JsonHelper.GetJsonElement(peData, "reference_no"));

                            double paidAmount = 0;
                            if (paidAmountEl.ValueKind == JsonValueKind.Number)
                                paidAmount = paidAmountEl.GetDouble();

                            payments.Add(new LinkedPayment
                            {
                                PaymentEntry = parentName,
            ...
            {
                Log.Error(ex, "GetLinkedPayments failed for {0}/{1}", doctype, docname);
            }
            return payments;
        }
    }
