    ...
    public class SocketIOHelper
    {
        ...
        public async Task<bool> ConnectToSocketIO()
        {
            try
            {
                ...
                // Create socket.io client with authentication using cookies
        ...
        /// <summary>
        /// Get authentication headers from stored cookies
        /// </summary>
        private Dictionary<string, string> GetAuthenticationHeaders()
        {
            ...
            try
            {
                // Extract cookies and add them to headers
                var uri = new Uri(Program.FrappeURL);
                var cookies = Program.Cookies.GetCookies(uri);

                if (cookies.Count > 0)
                {
                    var cookieHeader = string.Join("; ", cookies.Cast<System.Net.Cookie>()
                        .Select(c => $"{c.Name}={c.Value}"));

                    headers.Add("Cookie", cookieHeader);
                    Log.Debug("Added {0} cookies to Socket.IO connection", cookies.Count);
                }
                else
                {
                    Log.Warning("No cookies found for Socket.IO authentication");
                }
            }
            catch (Exception ex)
            {
                Log.Error(ex, "Error extracting cookies for Socket.IO: {0}", ex.Message);
            }
